...
List.prototype.fetch = function(req, res, context) {
  ...
  if (include.length) {
    options.include = _.merge([], include);
  }

  var searchParam = this.resource.search.param;
  if (_.has(req.query, searchParam)) {
    var searchParams = Array.isArray(req.query[searchParam]) ? req.query[searchParam] : [req.query[searchParam]];
    var searchAttributes =
          this.resource.search.attributes || Object.keys(model.rawAttributes);
    searchParams.forEach(function(term) {
      if (term.indexOf(':') > 0) {
        var field = term.split(':')[0];
        var value = term.split(':')[1];
        if (field.indexOf('.') < 0) {
          // Model field
          if (_.includes(searchAttributes, field)) {
            var modelFilter = {};
            modelFilter[field] = { like: '%' + value + '%' };
            if (Object.keys(criteria).length) {
              criteria = Sequelize.and(criteria, modelFilter);
            } else {
          ...
          // Associated field
          var findAssociatedModel;
          findAssociatedModel = function(includes, levels) {
            var includeToReturn;
            if (!includes) {
              return includeToReturn;
            }
            includes.forEach(function(include) {
              if ( (include.as && include.as === levels[0]) ||
                   (!include.as && include.model.name === levels[0]) ) {
                if (levels.length > 2) {
                  levels.shift();
                  includeToReturn = findAssociatedModel(include.include, levels);
                } else {
                  includeToReturn = include;
                }
              }
            });
            return includeToReturn;
          };
          var includeModel = findAssociatedModel(options.include, field.split('.'));
          if (includeModel) {
            var includeFilter = {};
            includeFilter[field.split('.').pop()] = { like: '%' + value + '%' };
            if (includeModel.where) {
              includeModel.where = Sequelize.and(includeModel.where, includeFilter);
            } else {
              includeModel.where = includeFilter;
            }
          }
        }
      } else {
