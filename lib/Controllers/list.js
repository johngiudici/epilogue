...
List.prototype.fetch = function(req, res, context) {
  ...
  if (_.has(req.query, searchParam)) {
    ...
    if (_.has(options, 'include') && !this.resource.search.attributes) {
      _.each(options.include, function(include) {
        ...
        var includeSearch = { $or: {} };
        _.each(Object.keys(include.model.rawAttributes), function(includeAttr) {
          includeSearch.$or[includeAttr] = { like: '%' + req.query[searchParam] + '%' };
        });

        include.where = includeSearch;
      });
    }
